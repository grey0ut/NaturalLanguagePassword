<#
.Synopsis
A Diceware password generating script
.Description
...
Param(
    [int32]$Pairs = 2,
    [ValidateSet("UpperCase","Substitution","Both")]
    [String]$Complexity = "Both",
    [String]$Append,
    [String]$Prepend,
    [Switch]$Shortcut
)
...
Function Get-PassAdjective {
    ...
    $RollResult = ''
         for ($RollNum = 1;$RollNum -le $NumDice; $RollNum++) {
            $RollResult = $RollResult += [string](Get-DiceRoll)
         }
    $Number = $RollResult.toint32($null)
...
<# Ver 1.1 method
If ($Complexity){
    (get-culture).textinfo.ToTitleCase($Outputobject) -replace "e","3" -replace "a","@" -replace "o","0" -replace "s","$" -replace "i","1"
}Else{
    $OutputObject
}
#>
...
If ($Shortcut){
    ...
    #Write-Host "Here is your passphrase" -ForegroundColor Cyan
    Write-Host $Line -BackgroundColor DarkBlue -ForegroundColor DarkCyan
    $OutputObject
    Write-Host $Line -BackgroundColor DarkBlue -ForegroundColor DarkCyan
    Write-Host " "
